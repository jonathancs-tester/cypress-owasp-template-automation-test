...
const envVars = JSON.parse(fs.readFileSync('cypress/html-info/html-info-'+ process.env.BROWSER +'/html-info.json'));
...
function getReportStatus() {
	...
	files.forEach(file => {
		...
		for(i in json[0].elements) {
			for(j in json[0].elements[i].steps) {
				var step = json[0].elements[i].steps[j]
				var stepStatus = step.result.status
				var stepDuration = step.result.duration
