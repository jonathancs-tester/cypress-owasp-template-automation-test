...
import step1Elements from '../elements/models/time-series/step1'
import indicatorsListElements from '../elements/feature_store/indcatorsList'
import myGroupsElements from '../elements/feature_store/myFavorites'

const MyGroupsElements = new myGroupsElements
const IndicatorsListElements = new indicatorsListElements
const Step1Elements = new step1Elements
const path = require('path')
class utils {

	checkDownloadButton(downloadButton, comparationSize, externalLink) {
		...
		if(externalLink == true)
			this.downloadExternalFile(downloadButton)		
		else
	...
	checkExportDisable(){
		cy.get(IndicatorsListElements.buttonExport()).should('have.attr', 'disabled')
	}

	downloadExternalFile(downloadButton){
	...
	checkInfoDownloadButton(downloadButton, column, value) {
		const downloadsFolder = Cypress.env('download_path')+'/'+Cypress.env('device')+'/'+Cypress.browser.name				
		//cy.task('clearFile', downloadsFolder) //alterarado pois estava encontrando dataset pra limpar
		cy.task('createDownloadFolder', Cypress.env('download_path'))
